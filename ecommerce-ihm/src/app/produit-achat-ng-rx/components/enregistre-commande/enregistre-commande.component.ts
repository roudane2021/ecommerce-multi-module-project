...
import { Observable, combineLatest, debounce, debounceTime, fromEvent, map, skip, startWith, tap } from 'rxjs';
...
export class EnregistreCommandeComponent implements OnInit{
  ...
  ngOnInit(): void {
   ...
   this.initFormObservables();
    const userActiveClick$ = fromEvent(document, 'click').pipe(map(data => ''),startWith(''));
    const userActiveMouseMove$ = fromEvent(document, 'mousemove').pipe(map(data => ''),startWith(''));
    const activity$ = combineLatest([userActiveClick$, userActiveMouseMove$]).pipe(debounceTime(4000));

    activity$.subscribe({
      'next' : (data) => {
             console.log(`User Inactive pendant 8 seconds : `)
             this.router.navigate(['produits/5']);
      } 
    })
  }
  ...
  initFormCtrls = () : void => {
    ...
    this.userNameCtrl = this.formBuilder.control('', {
      ...
      asyncValidators: [usernameValidator(this._userService)],
      updateOn: 'blur'
    });
